...
// Helper to get display label for sourceType
function getSourceTypeLabel(value: string): string {
    const module = ESG_MODULES.find(m => m.value === value);
...
interface AssetsTableProps {
    columns: ColumnDef<AssetTypology, unknown>[];
    data: AssetTypology[];
    isLoading?: boolean;
}
                ...
                <DropdownMenuContent align="start" className="w-56 max-h-64 overflow-y-auto">
                    ...
                    {uniqueValues.map((value) => {
                        ...
                        if (columnId === "sourceType") {
                            displayValue = getSourceTypeLabel(value);
                        } else if (columnId === "reportingFrequency") {
                            displayValue = getFrequencyLabel(value);
                        }
                        return (
                            <DropdownMenuCheckboxItem
                                key={value}
...
export function AssetsTable({ columns, data, isLoading }: AssetsTableProps) {
    ...
    const columnsWithFilters = useMemo((): ColumnDef<AssetTypology, unknown>[] => {
        return columns.map((col): ColumnDef<AssetTypology, unknown> => {
            ...
            if (accessorKey && FILTERABLE_COLUMNS.includes(accessorKey as FilterableColumn)) {
                ...
                } as ColumnDef<AssetTypology, unknown>;
            }
            return col;
        });
