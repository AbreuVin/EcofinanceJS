...
interface UnitsTableProps {
    columns: ColumnDef<Unit, unknown>[];
    data: Unit[];
    isLoading?: boolean;
}

// Columns that should have Excel-like filters
const FILTERABLE_COLUMNS = ["company.name", "country", "state", "city"] as const;

type FilterableColumn = typeof FILTERABLE_COLUMNS[number];

// Helper to get nested value from object
function getNestedValue(obj: Unit, path: string): string {
...
export function UnitsTable({ columns, data, isLoading }: UnitsTableProps) {
    ...
    const columnsWithFilters = useMemo((): ColumnDef<Unit, unknown>[] => {
        return columns.map((col): ColumnDef<Unit, unknown> => {
            ...
            {hasActiveFilters && (
                <div className="flex items-center gap-2 text-sm text-muted-foreground">
                    ...
                    {FILTERABLE_COLUMNS.map((columnId) => {
                        ...
                        const label = columnId === "company.name" ? "Empresa" : columnId === "country" ? "País" : columnId === "state" ? "Estado" : "Cidade";
                        return (
