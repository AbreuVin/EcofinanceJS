...
function hydrate(client, dehydratedState, options) {
	...
	mutations.forEach(({ state, ...mutationOptions$1 }) => {
...
function useQueries({ queries, ...options }, queryClient) {
	...
	defaultedQueries.forEach((queryOptions$1) => {
		ensureSuspenseTimers(queryOptions$1);
		ensurePreventErrorBoundaryRetry(queryOptions$1, errorResetBoundary, client.getQueryCache().get(queryOptions$1.queryHash));
	});
	...
	const suspensePromises = optimisticResult.some((result, index) => shouldSuspend(defaultedQueries[index], result)) ? optimisticResult.flatMap((result, index) => {
		...
		if (opts) {
			...
			if (shouldSuspend(opts, result)) return fetchOptimistic(opts, queryObserver, errorResetBoundary);
			else if (willFetch(result, isRestoring)) fetchOptimistic(opts, queryObserver, errorResetBoundary);
		}
		return [];
	}) : [];
